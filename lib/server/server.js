...
app.use((0, _middlewares2.default)(brain).authMiddleware);
...
app.post('/api/v1/voice', (0, _voice2.default)(brain).create);

var config = { host: 'http://laibulle.com:1234', subdomain: undefined };

if (brain.getServerId() != null) {
  config.subdomain = brain.getServerId();
}

var tunnel = (0, _localtunnel2.default)(port, config, function (err, tunnel) {
  if (err) {
    logger.error(err);return;
  }

  if (brain.getServerId() == null) {
    var foundUrl = _url2.default.parse(tunnel.url);
    if (foundUrl.host != null) {
      var sub = foundUrl.host.split('.', 1)[0];
