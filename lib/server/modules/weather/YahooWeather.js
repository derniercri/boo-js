    ...
    value: function update(sdk, location) {
      ...
      (0, _yahooWeather2.default)(location).then(function (info) {
        if (info != null) {
          ...
          _weather.set(parseTime(info.astronomy.sunrise), parseTime(info.astronomy.sunset), parseInt(info.item.condition.temp), info.item.condition.code, info.item.condition.text);
