    ...
    value: function handleTimeChange(event, scenes) {
      ...
      this.componentManager.getAllByType(_sdk.WEATHER_TYPE).map(function (item) {
        if (item.values.sunset != null && item.values.sunrise != null) {
          if (self.updateState(SUNSET, !(getTime(event.time) > getTime(_moment2.default.unix(item.values.sunrise)) && getTime(event.time) < getTime(_moment2.default.unix(item.values.sunset))))) {
            // Find SUNSET or SUNRISE events
            var lookup = _this.state.sunset == true ? _sdk.TRIGGER_SUNSET : _sdk.TRIGGER_SUNRISE;
